...
static int imx678_open(struct v4l2_subdev *sd, struct v4l2_subdev_fh *fh)
{
	...
	struct v4l2_mbus_framefmt *try_fmt_img =
		v4l2_subdev_state_get_format(sd, fh->state, IMAGE_PAD);
	struct v4l2_mbus_framefmt *try_fmt_meta =
		v4l2_subdev_state_get_format(sd, fh->state, METADATA_PAD);
	...
	try_crop = v4l2_subdev_state_get_crop(sd, fh->state, IMAGE_PAD);
				 ...
				 struct v4l2_subdev_format *fmt)
{
	...
	if (fmt->which == V4L2_SUBDEV_FORMAT_TRY) {
		struct v4l2_mbus_framefmt *try_fmt =
			v4l2_subdev_state_get_format(&imx678->sd, sd_state,
							fmt->pad);
				 ...
				 struct v4l2_subdev_format *fmt)
{
	...
	if (fmt->pad == IMAGE_PAD) {
		...
		if (fmt->which == V4L2_SUBDEV_FORMAT_TRY) {
			framefmt = v4l2_subdev_state_get_format(sd, sd_state,
								fmt->pad);
			*framefmt = fmt->format;
			...
			unsigned int pad, enum v4l2_subdev_format_whence which)
{
	...
	case V4L2_SUBDEV_FORMAT_TRY:
		return v4l2_subdev_state_get_crop(&imx678->sd, sd_state, pad);
